...
close all
comment='230324'
for model =19%fliplr(1:17)
    addpath R:\Felles_PCRTP\div
    clearvars -except model showSignal comment gradient_wonky
    %model = 3;
    N = 20; %pixel size
    ...
    switch model
        ...
        case 5.1
            ...
            Gy_strength = [zeros(1,10)  zeros(1,40) zeros(1,10) ones(1,20)  zeros(1,10)   zeros(1,80)   zeros(1,10) -ones(1,40)/2  zeros(1,10)]*0.05; %T/pixel
            T=length(Gx_strength)
            textString = 'GRE_readout_motionCompGrads';
        case 6
        ...
        case 16
            Gx_strength = [zeros(1,5) ones(1,40)...
                zeros(1,10) ... %RF pulse!
                zeros(1,10) zeros(1,10) ones(1,80) zeros(1,10) zeros(1,10)...
                zeros(1,10)... %RF pulse!
                zeros(1,10) zeros(1,10) ones(1,80) zeros(1,10) zeros(1,10)...
                zeros(1,10)... %RF pulse!
                zeros(1,10) zeros(1,10) ones(1,80) zeros(1,10) zeros(1,10)...
            ...
            Gy_strength = [zeros(1,5) zeros(1,40)...
                zeros(1,10)... %RF pulse!
                ones(1,10)*15   zeros(1,100) -ones(1,10)*15,...
                zeros(1,10)... %RF pulse!
                ones(1,10)*10   zeros(1,100) -ones(1,10)*10,...
                zeros(1,10)... %RF pulse!
                ones(1,10)*5   zeros(1,100) -ones(1,10)*5,...
                zeros(1,10)... %RF pulse!
                ones(1,10)*0   zeros(1,100) -ones(1,10)*0,...
                zeros(1,10)... %RF pulse!
                ones(1,10)*-5   zeros(1,100) -ones(1,10)*-5,...
                zeros(1,10)... %RF pulse!
                ones(1,10)*-10   zeros(1,100) -ones(1,10)*-10,...
            ...
            T2Decay = false;
             OneEightyPulse = [50 160+20 270+20+20 380+20+20+20 490+20+20+20+20 600+20+20+20+20+20 710+120 820+140];
        case 17
              Gx_strength = [zeros(1,5) ones(1,40)...
                zeros(1,10) ... %RF pulse!
                zeros(1,10) zeros(1,10) ones(1,80) zeros(1,10) zeros(1,10)...
        ...
        case 18
            ...
            Gy_strength = [zeros(1,10)  zeros(1,40) zeros(1,10) ones(1,20)  zeros(1,10)   zeros(1,80)   zeros(1,10) -ones(1,40)/2  zeros(1,10)]*0.05; %T/pixel
            T=length(Gx_strength)
            textString = 'GRE_readout_tiwhT2decay';
            T2Decay = true;
            t2factor = 0.99;
        ...
        case 19
            ...
            T=length(Gx_strength);
            textString = 'FSE_readout_centerOut_withT2decay';
            T2Decay = true;
             OneEightyPulse = [50 160+20 270+20+20 380+20+20+20 490+20+20+20+20 600+20+20+20+20+20 710+120 820+140];
             t2factor = 0.997;
    end
    ...
    if gradient_wonky
        Gx_strength = Gx_strength+0.2*(rand(size(Gx_strength))-.45)*.12;
        Gy_strength = Gy_strength+0.2*(rand(size(Gx_strength))-.5)*.12;
    end
    ...
    if movmake
        ...
        filename =strcat(['movies\sim_kspace_',comment,'_',textString,'_',num2str(randi(10000))]);
    ...
    t_to_run = unique(sort([round(linspace(1,T,200)) 1 T OneEightyPulse]));
        ...
        if ismember(t,OneEightyPulse)
            for tt=1:25
                ...
                if showSignal
                    ...
                    plot(kx(t-1)-2*kx(t-1)/25*tt,ky(t-1)-2*ky(t-1)/25*tt,'o','LineWidth',5,'MarkerSize',10,'Color',[1 0 0])
                    xlabel('kx')
                    ylabel('ky')
                    axis equal
                    set(gca,'Color','k')
                    ...
                    plot([0 0],[-1 1]*max([abs(kx) abs(ky)]),'-k')
                    plot(kx(1:t),ky(1:t),'k','LineWidth',2)
                    plot(kx(t-1)-2*kx(t-1)/25*tt,ky(t-1)-2*ky(t-1)/25*tt,'ok','MarkerSize',10,'MarkerFaceColor','k')
                    xlabel('kx')
                    ylabel('ky')
                    axis equal
                end
                ...
                for x=1:N
                    for y=1:N
                        ...
                        plot([0 real(theta(x,y,t-1))]*.4+x,[0 imag(theta(x,y,t-1))-2*imag(theta(x,y,t-1))/25*tt]*.4+y,'Color','k','LineWidth',5);
                        cmap = hot;
                        cmap=flipud(cmap);
                        cmap=cmap(20:end-20,:);
                ...
                axis([-1 N+1 -1 N+1])
                axis off
                title('Flipping... (this happens really fast in real life)')
                pause(.01)
                if movmake
