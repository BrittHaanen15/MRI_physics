...
showSignal=true %show kspace sigla as well
...
for model =9
    ...
    switch model
        ...
        case 10
          Gx_strength = [zeros(1,10)  +ones(1,40) zeros(1,10) ...
                ...
                zeros(1,5) ones(1,80) zeros(1,10)...
                zeros(1,5) -ones(1,80) zeros(1,10)...
                zeros(1,5) ones(1,80) zeros(1,10)]*0.05; %T/pixel
            ...
            Gy_strength = [zeros(1,10)  ones(1,40) zeros(1,10)... 
                ...
                -ones(1,5)/2   zeros(1,80) zeros(1,10)...
                -ones(1,5)/2   zeros(1,80) zeros(1,10)...
                -ones(1,5)/2   zeros(1,80) zeros(1,10)]*0.05; %T/pixel
            T=length(Gx_strength)
    ...
    for t=2:T
        kx(t) = gamma/2/pi*sum(Gx_strength(1:t));
        ky(t) = gamma/2/pi*sum(Gy_strength(1:t));
        for x=1:N
            for y=1:N
    ...
    for t=round(linspace(1,T,200))
        ...
        if showSignal
           ...
           for tt=1:t
               colorPower = 3*kSpaceSignal_f_of_t(tt)/max(kSpaceSignal_f_of_t);
               colorPower(colorPower>1)=1;
               
            plot(kx(tt),ky(tt),'ok','MarkerSize',5,'MarkerFaceColor',[1 1 1]*colorPower)
           end
           % plot(kx(t),ky(t),'ok','MarkerSize',10,'MarkerFaceColor',[1 1 1]*kSpaceSignal_f_of_t/max(kSpaceSignal_f_of_t))
            plot(kx(t),ky(t),'ok','MarkerSize',5,'Color',[1 0 0])
            xlabel('kx')
            ylabel('ky')
            axis equal
set(gca,'Color','k')
