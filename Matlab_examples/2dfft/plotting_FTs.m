...
title('2DFT of 12x12 face (only abs value shown)')
axis equal off
colormap(bone)
...
%% Block function and truncation
%a
clear
Fs = 1000000; %sampling frequency (1/s)
T = 1/Fs; % sampling period (s)
duration = 1 %s
N = duration*Fs; %lenght of signal
...
switch curve
    case 1  
        ...
        s(isnan(s))=1;
    case 2 %keep only middle 8 s
        b=3000 %3 kHz
        s=sin(tvec*2*pi*b)./(tvec*2*pi*b);
        s(isnan(s))=1;
        s(tvec<-4e-3) = 0;
        s(tvec>4e-3) = 0;
    case 3 %keep only middle 8 s
        b=3000 %3 kHz
        s=sin(tvec*2*pi*b)./(tvec*2*pi*b);
        s(isnan(s))=1;
        s(tvec<-1e-3) = 0;
    ...
    case 4
        b=3000 %3 kHz
        d=10000
        s=sin(tvec*2*pi*b)./(tvec*2*pi*b).*cos(2*pi*tvec*d);
        s(isnan(s))=1;
        s(tvec<-1e-3) = 0;
        s(tvec>1e-3) = 0;
        s(isnan(s))=1;
